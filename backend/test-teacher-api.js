...
async function testTeacherAPI() {
    try {
        ...
        // Get all unique classes from approved students
        const allClasses = await ApprovedStudent.distinct('grade', { teacherId: teacher._id });
        console.log('Available classes for this teacher:', allClasses);
        
        if (allClasses.length > 0) {
            const testClass = allClasses[0]; // Test with first available class
            ...
            console.log('No classes found for this teacher. Teacher needs to add students first.');
        }
